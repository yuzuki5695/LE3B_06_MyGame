...
namespace MyGame {
    void Pausemenu::Initialize() {
        TextureManager::GetInstance()->LoadTexture(Textures::Pausemenu::PauseMenuPanel);
        TextureManager::GetInstance()->LoadTexture(Textures::Pausemenu::Poseicon);
        TextureManager::GetInstance()->LoadTexture(Textures::Pausemenu::Poseicon_red);
        TextureManager::GetInstance()->LoadTexture(Textures::Pausemenu::menu_01);
        TextureManager::GetInstance()->LoadTexture(Textures::Pausemenu::menu_02);
        TextureManager::GetInstance()->LoadTexture(Textures::Pausemenu::menu_03);
        TextureManager::GetInstance()->LoadTexture(Textures::Pausemenu::menu_04);
        TextureManager::GetInstance()->LoadTexture(Textures::Pausemenu::Arrow);

        // 初期状態は非アクティブ
        isActive_ = false;
        // ポーズアイコンの作成
        paused_ = Sprite::Create(Textures::Pausemenu::Poseicon, Vector2{ 3.0f, 3.0f }, 0.0f, Vector2{ 100.0f,100.0f });
        ...
        for (uint32_t i = 0; i < 2; i++) {
            ...
            startPos.y = 1000.0f;
            element.sprite = Sprite::Create(Textures::Pausemenu::PauseMenuPanel, startPos, 0.0f, element.size);
        ...
        // テクスチャ読み込み
        std::string textures[] = {
            Textures::Pausemenu::menu_01, Textures::Pausemenu::menu_03,
            Textures::Pausemenu::menu_02, Textures::Pausemenu::menu_04
        };
        // コマンドUIの作成
        ...
        arrow_.sprite = Sprite::Create(Textures::Pausemenu::Arrow, arrow_.position, 0.0f, arrow_.size);
    ...
    void Pausemenu::Update() {
        if (isOpening_) {
            ...
            for (uint32_t i = 0; i < panels_.size(); i++) {
                ...
                // サイズ補間
                Vector2 currentSize{ targetSize.x * easeT,                    targetSize.y * easeT };
        ...
        if (isActive_) {
            paused_->SetTexture(Textures::Pausemenu::Poseicon_red);
            keyUpdate();
            ExecuteCommand(); // コマンド実行
        } else {
            paused_->SetTexture(Textures::Pausemenu::Poseicon);
        }
    ...
    void Pausemenu::ExecuteCommand() {
        ...
        if (!isOperationMove_ && Input::GetInstance()->TriggerKey(DIK_RETURN)) {
            ...
            case PauseCommand::GoToTitle:
                ...
                // フェードアウト
                FadeManager::GetInstance()->SceneChangeFade(SceneName::TITLE, FadeStyle::Normal, 1.0f);
